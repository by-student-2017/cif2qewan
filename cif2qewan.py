...
class qe_wannier_in:
    ...
    def convert2nscf(self):
        ...
        if(self.so or self.mag):
            ...
            system_add_str += "  nbnd = {}\n".format(int(self.nexclude + self.num_wann*self.num_wann_times*1.1)*2)
        else:
    ...
    def shift_k_nscf(self):
        ...
        if(self.so or self.mag):
            ...
            nbnd = int(self.nexclude + self.num_wann*self.num_wann_times*1.1)*2
        else:
            #nbnd = self.nexclude + int(self.num_wann*1.5)
            nbnd = int(self.nexclude + self.num_wann*self.num_wann_times*1.1)
    ...
    def write_wannier(self, wannier_in):
        with open(wannier_in, "w") as fp:
            ...
            fp.write("num_bands = {}\n".format(int(self.num_wann*self.num_wann_times*1.1)*so_factor))
