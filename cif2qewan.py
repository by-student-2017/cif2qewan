...
class qe_wannier_in:
    ...
    def convert2nscf(self):
        ...
        if(self.so or self.mag):
            ...
            system_add_str += "  nbnd = {}\n".format(int(self.nexclude + self.num_wann*1.1+4)*2)
        else:
            #system_add_str += "  nbnd = {}\n".format(self.nexclude + self.num_wann*3)
            system_add_str += "  nbnd = {}\n".format(int(self.nexclude + self.num_wann*1.1+4))
    ...
    def shift_k_nscf(self):
        ...
        if(self.so or self.mag):
            ...
            nbnd = int(self.nexclude + self.num_wann*1.1+4)*2
        else:
            #nbnd = self.nexclude + int(self.num_wann*1.5)
            nbnd = int(self.nexclude + self.num_wann*1.1+4)
